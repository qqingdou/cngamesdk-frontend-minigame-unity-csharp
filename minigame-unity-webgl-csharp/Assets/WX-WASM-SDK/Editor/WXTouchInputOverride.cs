...
public class WechatTouchInputOverride : BaseInput
{
	...
	private void InitWechatTouchEvents()
	{
		if (!_isInitWechatSDK)
		{
			WX.InitSDK((code) =>
			{
				...
				Debug.LogFormat("WX.InitSDK:{0}", code);
	...
	private TouchData FindOrCreateTouchData(long identifier)
	{
		...
		data.touch.fingerId = (int)identifier;
		data.touch.radius = 0;
